...
#pragma HLS ARRAY_PARTITION variable=ALPHAmn_ complete dim=1
	...
	FILE *fp3 = fopen("C:/Dropbox/codes/non_binary_LDPC/vs/minmax/B.txt","w");
...
#pragma HLS ARRAY_PARTITION variable=GAMMAn_post complete dim=1
	...
	{
		col = col_row[row][0];
		minmax_label23:for(index = 0; index < row_weight[row] ; index++)
		{
#pragma HLS LOOP_TRIPCOUNT min=3 max=6
...
#pragma HLS LOOP_TRIPCOUNT min=6 max=6
	...
	// 	 processing: update variable node message ALPHA
			...
			minmax_label132:while(index_v < (col_weight[col_v])){
...
#pragma HLS unroll
	...
	// post processing
			...
			minmax_label22:for( a = 0; a < Q; a++){
				...
				index_ = 0;
				row_ = row_col[0][col];
				minmax_label723:while(index_ <col_weight[col] ){
#pragma HLS LOOP_TRIPCOUNT min=6 max=6
