...
{
#pragma HLS RESOURCE variable=BETAmn_ core=RAM_2P_BRAM
#pragma HLS RESOURCE variable=GAMMAn_ core=ROM_nP_BRAM
#pragma HLS RESOURCE variable=ALPHAmn_ core=RAM_2P_BRAM
#pragma HLS ARRAY_PARTITION variable=ALPHAmn_ complete dim=3
#pragma HLS ARRAY_PARTITION variable=GAMMAn_ complete dim=2
#pragma HLS ARRAY_PARTITION variable=BETAmn_ complete dim=3
	...
	unsigned char ALPHA_t[Q];
#pragma HLS ARRAY_PARTITION variable=ALPHA_t complete dim=1

	unsigned char codeword_sym[V];
#pragma HLS ARRAY_PARTITION variable=codeword_sym complete dim=1
	unsigned char B[Q][Y];
	unsigned char F[Q][Y];
...
#pragma HLS unroll
		for(j = 0; j < V; j++){
#pragma HLS unroll
#pragma HLS ARRAY_PARTITION variable=GAMMAn_post complete dim=2
			GAMMAn_post[i][j] = 0.0;
		}
	}
