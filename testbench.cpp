...
#ifdef QC
	fp = fopen("C:/Users/mtech/Desktop/cpu_minmax/cpu_minmax/matrix/qc.G'.nb.txt","w+");
	fpp = fopen("C:/Users/mtech/Desktop/cpu_minmax/cpu_minmax/matrix/qc.H'.nb.txt","w+");
#else
	if(Q==16)
	{
...
void sym_llr(double* llr_, unsigned char BETAmn_[U][V][Q], unsigned char GAMMAn_[Q][V], int* hard, double* recv, double sigma)
{
	...
	for(i = 0; i < M;i++){	// initialize channel information
...
int main(int argc, char*argv[])
{
	...
	for (snr = SNR_LOW; snr <= SNR_HIGH; snr += SNR_DELTA)
	{
...
#endif
		{
			...
			fprintf(stderr, "throughput is %f Kbps\n", counter * U* 5/total_time);

			error_num += error;
			if(error != 0) fe++;
			printf("frame = %d frame error = %d\n",counter, fe);
		}
