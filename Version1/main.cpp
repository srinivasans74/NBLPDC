#include "stdio.h"
#include "stdlib.h"
#include <math.h>
#include <time.h>
#include <string.h>
//#include <ap_fixed.h>
#include "header.h"
#ifdef VS
#define _CRT_SECURE_NO_DEPRECATE
#endif

//===================================
// Allocate 2D array of double
//===================================
double **malloc2Ddouble(int arraySizeX, int arraySizeY) // allocates array[a][b]
{
	double** theArray;  
	theArray = (double**) malloc(arraySizeX*sizeof(double*));  
	for (int i = 0; i < arraySizeX; i++)  
		theArray[i] = (double*) malloc(arraySizeY*sizeof(double));  
	return theArray; 
}

//===================================
// Galios Field Multiplication
//===================================
unsigned char mul_gf(unsigned char a, unsigned char b)
{
#pragma HLS RESOURCE variable=logq core=ROM_2P_BRAM
#pragma HLS RESOURCE variable=expq core=ROM_2P_BRAM

	if (a == 0 || b == 0) return 0;
	if (a == 1) return b;
	if (b == 1) return a;
	if ((logq[a] + logq[b])>(Q-1))
			{
			return	expq[(logq[a] + logq[b])- (Q-1)];
			}
			else
			return expq[(logq[a] + logq[b])];
			///table look-up multiplication


/*	if (a == 0 || b == 0) return 0;
		if (a == 1) return b;
		if (b == 1) return a;
		return expq[(logq[a] + logq[b]) % (Q-1)];
*/
}

//===================================
// Galios Field Inverse
//===================================
unsigned char inv_gf(unsigned char a)
{
#pragma HLS RESOURCE variable=logq core=ROM_2P_BRAM
#pragma HLS RESOURCE variable=expq core=ROM_2P_BRAM
	//if(a == 0) exit (EXIT_FAILURE);
	if(a == 0) return 0;
	if(a == 1) return 1;
	return expq[(Q - 1 - logq[a])];
	//return expq[(Q - 1 - logq[a]) % (Q-1)];

}

//===================================
// Galios Field Multiplication
//===================================
int div_gf(int a, int b)
{

	if (a == 0) return 0;
	if (b == 1) return a;
	return expq[(logq[a] - logq[b] + (Q-1))% (Q-1)];	//table look-up multiplication
}


//===================================
// Minmax nonbinary decode function
//===================================
// Refer to "Min-max decoding for nonbinary LDPC codes"
int minmax(unsigned char GAMMAn_[Q][V])
{
#pragma HLS ARRAY_PARTITION variable=GAMMAn_ complete dim=1
unsigned char	BETAmn_1[U][V][Q];
#pragma HLS RESOURCE variable=BETAmn_1 core=RAM_2P_BRAM
#pragma HLS ARRAY_PARTITION variable=BETAmn_1 complete dim=3
unsigned char	ALPHAmn_1[U][V][Q];
#pragma HLS ARRAY_PARTITION variable=ALPHAmn_1 complete dim=3
#pragma HLS RESOURCE variable=ALPHAmn_1 core=RAM_2P_BRAM
int row_col1[3][300];
#pragma HLS ARRAY_PARTITION variable=row_col1 complete dim=1
int col_row1[150][6];
//#pragma HLS ARRAY_PARTITION variable=col_row1 complete dim=1
unsigned char H1[150][300];

#pragma HLS INTERFACE m_axi depth=38400 port=GAMMAn_
//#pragma HLS ARRAY_PARTITION variable=BETAmn_1 complete dim=3

	int row,col,index,index_c,index_v,col_v,row_v,error;
	int index_, row_;
	int last_col;
	int index_B, col_B;
	int index_p, row_p, col_p;
	int i,j;
	int a,b,c;
	unsigned char iter = 0;
	int min_index;
	unsigned char min_F,min_B;
	unsigned char max_F,max_B;
	unsigned char temp;
	unsigned char min_value,max_value;
	unsigned char ALPHA_t[Q];
//#pragma HLS RESOURCE variable=ALPHA_t core=RAM_2P_BRAM


int rowweight2[U];
//pragma HLS RESOURCE variable=rowweight2 core=RAM_T2P_URAM
/*
for(i=0;i<U;i++)
{
#pragma HLS unroll
	 rowweight2[i]=6;

}
for (i=0;i<150;i++)
{
//#pragma HLS unroll
	for (j=0;j<300;j++)
	{
#pragma HLS unroll
		H1[i][j]=0;
	}
}
/*
H1[0][8]= 5;
H1[0][122]= 4;
H1[0][149]= 9;
H1[0][155]= 6;
H1[0][276]= 12;
H1[0][288]= 8;
H1[1][9]= 10;
H1[1][123]= 8;
H1[1][135]= 1;
H1[1][156]= 12;
H1[1][277]= 11;
H1[1][289]= 3;
H1[2][10]= 7;
H1[2][124]= 3;
H1[2][136]= 2;
H1[2][157]= 11;
H1[2][278]= 5;
H1[2][290]= 6;
H1[3][11]= 14;
H1[3][125]= 6;
H1[3][137]= 4;
H1[3][158]= 5;
H1[3][279]= 10;
H1[3][291]= 12;
H1[4][12]= 15;
H1[4][126]= 12;
H1[4][138]= 8;
H1[4][159]= 10;
H1[4][280]= 7;
H1[4][292]= 11;
H1[5][13]= 13;
H1[5][127]= 11;
H1[5][139]= 3;
H1[5][160]= 7;
H1[5][281]= 14;
H1[5][293]= 5;
H1[6][14]= 9;
H1[6][128]= 5;
H1[6][140]= 6;
H1[6][161]= 14;
H1[6][282]= 15;
H1[6][294]= 10;
H1[7][0]= 1;
H1[7][129]= 10;
H1[7][141]= 12;
H1[7][162]= 15;
H1[7][283]= 13;
H1[7][295]= 7;
H1[8][1]= 2;
H1[8][130]= 7;
H1[8][142]= 11;
H1[8][163]= 13;
H1[8][284]= 9;
H1[8][296]= 14;
H1[9][2]= 4;
H1[9][131]= 14;
H1[9][143]= 5;
H1[9][164]= 9;
H1[9][270]= 1;
H1[9][297]= 15;
H1[10][3]= 8;
H1[10][132]= 15;
H1[10][144]= 10;
H1[10][150]= 1;
H1[10][271]= 2;
H1[10][298]= 13;
H1[11][4]= 3;
H1[11][133]= 13;
H1[11][145]= 7;
H1[11][151]= 2;
H1[11][272]= 4;
H1[11][299]= 9;
H1[12][5]= 6;
H1[12][134]= 9;
H1[12][146]= 14;
H1[12][152]= 4;
H1[12][273]= 8;
H1[12][285]= 1;
H1[13][6]= 12;
H1[13][120]= 1;
H1[13][147]= 15;
H1[13][153]= 8;
H1[13][274]= 3;
H1[13][286]= 2;
H1[14][7]= 11;
H1[14][121]= 2;
H1[14][148]= 13;
H1[14][154]= 3;
H1[14][275]= 6;
H1[14][287]= 4;
H1[15][1]= 2;
H1[15][23]= 5;
H1[15][137]= 4;
H1[15][164]= 9;
H1[15][170]= 6;
H1[15][291]= 12;
H1[16][2]= 4;
H1[16][24]= 10;
H1[16][138]= 8;
H1[16][150]= 1;
H1[16][171]= 12;
H1[16][292]= 11;
H1[17][3]= 8;
H1[17][25]= 7;
H1[17][139]= 3;
H1[17][151]= 2;
H1[17][172]= 11;
H1[17][293]= 5;
H1[18][4]= 3;
H1[18][26]= 14;
H1[18][140]= 6;
H1[18][152]= 4;
H1[18][173]= 5;
H1[18][294]= 10;
H1[19][5]= 6;
H1[19][27]= 15;
H1[19][141]= 12;
H1[19][153]= 8;
H1[19][174]= 10;
H1[19][295]= 7;
H1[20][6]= 12;
H1[20][28]= 13;
H1[20][142]= 11;
H1[20][154]= 3;
H1[20][175]= 7;
H1[20][296]= 14;
H1[21][7]= 11;
H1[21][29]= 9;
H1[21][143]= 5;
H1[21][155]= 6;
H1[21][176]= 14;
H1[21][297]= 15;
H1[22][8]= 5;
H1[22][15]= 1;
H1[22][144]= 10;
H1[22][156]= 12;
H1[22][177]= 15;
H1[22][298]= 13;
H1[23][9]= 10;
H1[23][16]= 2;
H1[23][145]= 7;
H1[23][157]= 11;
H1[23][178]= 13;
H1[23][299]= 9;
H1[24][10]= 7;
H1[24][17]= 4;
H1[24][146]= 14;
H1[24][158]= 5;
H1[24][179]= 9;
H1[24][285]= 1;
H1[25][11]= 14;
H1[25][18]= 8;
H1[25][147]= 15;
H1[25][159]= 10;
H1[25][165]= 1;
H1[25][286]= 2;
H1[26][12]= 15;
H1[26][19]= 3;
H1[26][148]= 13;
H1[26][160]= 7;
H1[26][166]= 2;
H1[26][287]= 4;
H1[27][13]= 13;
H1[27][20]= 6;
H1[27][149]= 9;
H1[27][161]= 14;
H1[27][167]= 4;
H1[27][288]= 8;
H1[28][14]= 9;
H1[28][21]= 12;
H1[28][135]= 1;
H1[28][162]= 15;
H1[28][168]= 8;
H1[28][289]= 3;
H1[29][0]= 1;
H1[29][22]= 11;
H1[29][136]= 2;
H1[29][163]= 13;
H1[29][169]= 3;
H1[29][290]= 6;
H1[30][10]= 7;
H1[30][16]= 2;
H1[30][38]= 5;
H1[30][152]= 4;
H1[30][179]= 9;
H1[30][185]= 6;
H1[31][11]= 14;
H1[31][17]= 4;
H1[31][39]= 10;
H1[31][153]= 8;
H1[31][165]= 1;
H1[31][186]= 12;
H1[32][12]= 15;
H1[32][18]= 8;
H1[32][40]= 7;
H1[32][154]= 3;
H1[32][166]= 2;
H1[32][187]= 11;
H1[33][13]= 13;
H1[33][19]= 3;
H1[33][41]= 14;
H1[33][155]= 6;
H1[33][167]= 4;
H1[33][188]= 5;
H1[34][14]= 9;
H1[34][20]= 6;
H1[34][42]= 15;
H1[34][156]= 12;
H1[34][168]= 8;
H1[34][189]= 10;
H1[35][0]= 1;
H1[35][21]= 12;
H1[35][43]= 13;
H1[35][157]= 11;
H1[35][169]= 3;
H1[35][190]= 7;
H1[36][1]= 2;
H1[36][22]= 11;
H1[36][44]= 9;
H1[36][158]= 5;
H1[36][170]= 6;
H1[36][191]= 14;
H1[37][2]= 4;
H1[37][23]= 5;
H1[37][30]= 1;
H1[37][159]= 10;
H1[37][171]= 12;
H1[37][192]= 15;
H1[38][3]= 8;
H1[38][24]= 10;
H1[38][31]= 2;
H1[38][160]= 7;
H1[38][172]= 11;
H1[38][193]= 13;
H1[39][4]= 3;
H1[39][25]= 7;
H1[39][32]= 4;
H1[39][161]= 14;
H1[39][173]= 5;
H1[39][194]= 9;
H1[40][5]= 6;
H1[40][26]= 14;
H1[40][33]= 8;
H1[40][162]= 15;
H1[40][174]= 10;
H1[40][180]= 1;
H1[41][6]= 12;
H1[41][27]= 15;
H1[41][34]= 3;
H1[41][163]= 13;
H1[41][175]= 7;
H1[41][181]= 2;
H1[42][7]= 11;
H1[42][28]= 13;
H1[42][35]= 6;
H1[42][164]= 9;
H1[42][176]= 14;
H1[42][182]= 4;
H1[43][8]= 5;
H1[43][29]= 9;
H1[43][36]= 12;
H1[43][150]= 1;
H1[43][177]= 15;
H1[43][183]= 8;
H1[44][9]= 10;
H1[44][15]= 1;
H1[44][37]= 11;
H1[44][151]= 2;
H1[44][178]= 13;
H1[44][184]= 3;
H1[45][25]= 7;
H1[45][31]= 2;
H1[45][53]= 5;
H1[45][167]= 4;
H1[45][194]= 9;
H1[45][200]= 6;
H1[46][26]= 14;
H1[46][32]= 4;
H1[46][54]= 10;
H1[46][168]= 8;
H1[46][180]= 1;
H1[46][201]= 12;
H1[47][27]= 15;
H1[47][33]= 8;
H1[47][55]= 7;
H1[47][169]= 3;
H1[47][181]= 2;
H1[47][202]= 11;
H1[48][28]= 13;
H1[48][34]= 3;
H1[48][56]= 14;
H1[48][170]= 6;
H1[48][182]= 4;
H1[48][203]= 5;
H1[49][29]= 9;
H1[49][35]= 6;
H1[49][57]= 15;
H1[49][171]= 12;
H1[49][183]= 8;
H1[49][204]= 10;
H1[50][15]= 1;
H1[50][36]= 12;
H1[50][58]= 13;
H1[50][172]= 11;
H1[50][184]= 3;
H1[50][205]= 7;
H1[51][16]= 2;
H1[51][37]= 11;
H1[51][59]= 9;
H1[51][173]= 5;
H1[51][185]= 6;
H1[51][206]= 14;
H1[52][17]= 4;
H1[52][38]= 5;
H1[52][45]= 1;
H1[52][174]= 10;
H1[52][186]= 12;
H1[52][207]= 15;
H1[53][18]= 8;
H1[53][39]= 10;
H1[53][46]= 2;
H1[53][175]= 7;
H1[53][187]= 11;
H1[53][208]= 13;
H1[54][19]= 3;
H1[54][40]= 7;
H1[54][47]= 4;
H1[54][176]= 14;
H1[54][188]= 5;
H1[54][209]= 9;
H1[55][20]= 6;
H1[55][41]= 14;
H1[55][48]= 8;
H1[55][177]= 15;
H1[55][189]= 10;
H1[55][195]= 1;
H1[56][21]= 12;
H1[56][42]= 15;
H1[56][49]= 3;
H1[56][178]= 13;
H1[56][190]= 7;
H1[56][196]= 2;
H1[57][22]= 11;
H1[57][43]= 13;
H1[57][50]= 6;
H1[57][179]= 9;
H1[57][191]= 14;
H1[57][197]= 4;
H1[58][23]= 5;
H1[58][44]= 9;
H1[58][51]= 12;
H1[58][165]= 1;
H1[58][192]= 15;
H1[58][198]= 8;
H1[59][24]= 10;
H1[59][30]= 1;
H1[59][52]= 11;
H1[59][166]= 2;
H1[59][193]= 13;
H1[59][199]= 3;
H1[60][40]= 7;
H1[60][46]= 2;
H1[60][68]= 5;
H1[60][182]= 4;
H1[60][209]= 9;
H1[60][215]= 6;
H1[61][41]= 14;
H1[61][47]= 4;
H1[61][69]= 10;
H1[61][183]= 8;
H1[61][195]= 1;
H1[61][216]= 12;
H1[62][42]= 15;
H1[62][48]= 8;
H1[62][70]= 7;
H1[62][184]= 3;
H1[62][196]= 2;
H1[62][217]= 11;
H1[63][43]= 13;
H1[63][49]= 3;
H1[63][71]= 14;
H1[63][185]= 6;
H1[63][197]= 4;
H1[63][218]= 5;
H1[64][44]= 9;
H1[64][50]= 6;
H1[64][72]= 15;
H1[64][186]= 12;
H1[64][198]= 8;
H1[64][219]= 10;
H1[65][30]= 1;
H1[65][51]= 12;
H1[65][73]= 13;
H1[65][187]= 11;
H1[65][199]= 3;
H1[65][220]= 7;
H1[66][31]= 2;
H1[66][52]= 11;
H1[66][74]= 9;
H1[66][188]= 5;
H1[66][200]= 6;
H1[66][221]= 14;
H1[67][32]= 4;
H1[67][53]= 5;
H1[67][60]= 1;
H1[67][189]= 10;
H1[67][201]= 12;
H1[67][222]= 15;
H1[68][33]= 8;
H1[68][54]= 10;
H1[68][61]= 2;
H1[68][190]= 7;
H1[68][202]= 11;
H1[68][223]= 13;
H1[69][34]= 3;
H1[69][55]= 7;
H1[69][62]= 4;
H1[69][191]= 14;
H1[69][203]= 5;
H1[69][224]= 9;
H1[70][35]= 6;
H1[70][56]= 14;
H1[70][63]= 8;
H1[70][192]= 15;
H1[70][204]= 10;
H1[70][210]= 1;
H1[71][36]= 12;
H1[71][57]= 15;
H1[71][64]= 3;
H1[71][193]= 13;
H1[71][205]= 7;
H1[71][211]= 2;
H1[72][37]= 11;
H1[72][58]= 13;
H1[72][65]= 6;
H1[72][194]= 9;
H1[72][206]= 14;
H1[72][212]= 4;
H1[73][38]= 5;
H1[73][59]= 9;
H1[73][66]= 12;
H1[73][180]= 1;
H1[73][207]= 15;
H1[73][213]= 8;
H1[74][39]= 10;
H1[74][45]= 1;
H1[74][67]= 11;
H1[74][181]= 2;
H1[74][208]= 13;
H1[74][214]= 3;
H1[75][47]= 4;
H1[75][74]= 9;
H1[75][83]= 5;
H1[75][201]= 12;
H1[75][213]= 8;
H1[75][230]= 6;
H1[76][48]= 8;
H1[76][60]= 1;
H1[76][84]= 10;
H1[76][202]= 11;
H1[76][214]= 3;
H1[76][231]= 12;
H1[77][49]= 3;
H1[77][61]= 2;
H1[77][85]= 7;
H1[77][203]= 5;
H1[77][215]= 6;
H1[77][232]= 11;
H1[78][50]= 6;
H1[78][62]= 4;
H1[78][86]= 14;
H1[78][204]= 10;
H1[78][216]= 12;
H1[78][233]= 5;
H1[79][51]= 12;
H1[79][63]= 8;
H1[79][87]= 15;
H1[79][205]= 7;
H1[79][217]= 11;
H1[79][234]= 10;
H1[80][52]= 11;
H1[80][64]= 3;
H1[80][88]= 13;
H1[80][206]= 14;
H1[80][218]= 5;
H1[80][235]= 7;
H1[81][53]= 5;
H1[81][65]= 6;
H1[81][89]= 9;
H1[81][207]= 15;
H1[81][219]= 10;
H1[81][236]= 14;
H1[82][54]= 10;
H1[82][66]= 12;
H1[82][75]= 1;
H1[82][208]= 13;
H1[82][220]= 7;
H1[82][237]= 15;
H1[83][55]= 7;
H1[83][67]= 11;
H1[83][76]= 2;
H1[83][209]= 9;
H1[83][221]= 14;
H1[83][238]= 13;
H1[84][56]= 14;
H1[84][68]= 5;
H1[84][77]= 4;
H1[84][195]= 1;
H1[84][222]= 15;
H1[84][239]= 9;
H1[85][57]= 15;
H1[85][69]= 10;
H1[85][78]= 8;
H1[85][196]= 2;
H1[85][223]= 13;
H1[85][225]= 1;
H1[86][58]= 13;
H1[86][70]= 7;
H1[86][79]= 3;
H1[86][197]= 4;
H1[86][224]= 9;
H1[86][226]= 2;
H1[87][59]= 9;
H1[87][71]= 14;
H1[87][80]= 6;
H1[87][198]= 8;
H1[87][210]= 1;
H1[87][227]= 4;
H1[88][45]= 1;
H1[88][72]= 15;
H1[88][81]= 12;
H1[88][199]= 3;
H1[88][211]= 2;
H1[88][228]= 8;
H1[89][46]= 2;
H1[89][73]= 13;
H1[89][82]= 11;
H1[89][200]= 6;
H1[89][212]= 4;
H1[89][229]= 3;
H1[90][62]= 4;
H1[90][76]= 2;
H1[90][98]= 5;
H1[90][216]= 12;
H1[90][239]= 9;
H1[90][245]= 6;
H1[91][63]= 8;
H1[91][77]= 4;
H1[91][99]= 10;
H1[91][217]= 11;
H1[91][225]= 1;
H1[91][246]= 12;
H1[92][64]= 3;
H1[92][78]= 8;
H1[92][100]= 7;
H1[92][218]= 5;
H1[92][226]= 2;
H1[92][247]= 11;
H1[93][65]= 6;
H1[93][79]= 3;
H1[93][101]= 14;
H1[93][219]= 10;
H1[93][227]= 4;
H1[93][248]= 5;
H1[94][66]= 12;
H1[94][80]= 6;
H1[94][102]= 15;
H1[94][220]= 7;
H1[94][228]= 8;
H1[94][249]= 10;
H1[95][67]= 11;
H1[95][81]= 12;
H1[95][103]= 13;
H1[95][221]= 14;
H1[95][229]= 3;
H1[95][250]= 7;
H1[96][68]= 5;
H1[96][82]= 11;
H1[96][104]= 9;
H1[96][222]= 15;
H1[96][230]= 6;
H1[96][251]= 14;
H1[97][69]= 10;
H1[97][83]= 5;
H1[97][90]= 1;
H1[97][223]= 13;
H1[97][231]= 12;
H1[97][252]= 15;
H1[98][70]= 7;
H1[98][84]= 10;
H1[98][91]= 2;
H1[98][224]= 9;
H1[98][232]= 11;
H1[98][253]= 13;
H1[99][71]= 14;
H1[99][85]= 7;
H1[99][92]= 4;
H1[99][210]= 1;
H1[99][233]= 5;
H1[99][254]= 9;
H1[100][72]= 15;
H1[100][86]= 14;
H1[100][93]= 8;
H1[100][211]= 2;
H1[100][234]= 10;
H1[100][240]= 1;
H1[101][73]= 13;
H1[101][87]= 15;
H1[101][94]= 3;
H1[101][212]= 4;
H1[101][235]= 7;
H1[101][241]= 2;
H1[102][74]= 9;
H1[102][88]= 13;
H1[102][95]= 6;
H1[102][213]= 8;
H1[102][236]= 14;
H1[102][242]= 4;
H1[103][60]= 1;
H1[103][89]= 9;
H1[103][96]= 12;
H1[103][214]= 3;
H1[103][237]= 15;
H1[103][243]= 8;
H1[104][61]= 2;
H1[104][75]= 1;
H1[104][97]= 11;
H1[104][215]= 6;
H1[104][238]= 13;
H1[104][244]= 3;
H1[105][85]= 7;
H1[105][91]= 2;
H1[105][113]= 5;
H1[105][227]= 4;
H1[105][254]= 9;
H1[105][260]= 6;
H1[106][86]= 14;
H1[106][92]= 4;
H1[106][114]= 10;
H1[106][228]= 8;
H1[106][240]= 1;
H1[106][261]= 12;
H1[107][87]= 15;
H1[107][93]= 8;
H1[107][115]= 7;
H1[107][229]= 3;
H1[107][241]= 2;
H1[107][262]= 11;
H1[108][88]= 13;
H1[108][94]= 3;
H1[108][116]= 14;
H1[108][230]= 6;
H1[108][242]= 4;
H1[108][263]= 5;
H1[109][89]= 9;
H1[109][95]= 6;
H1[109][117]= 15;
H1[109][231]= 12;
H1[109][243]= 8;
H1[109][264]= 10;
H1[110][75]= 1;
H1[110][96]= 12;
H1[110][118]= 13;
H1[110][232]= 11;
H1[110][244]= 3;
H1[110][265]= 7;
H1[111][76]= 2;
H1[111][97]= 11;
H1[111][119]= 9;
H1[111][233]= 5;
H1[111][245]= 6;
H1[111][266]= 14;
H1[112][77]= 4;
H1[112][98]= 5;
H1[112][105]= 1;
H1[112][234]= 10;
H1[112][246]= 12;
H1[112][267]= 15;
H1[113][78]= 8;
H1[113][99]= 10;
H1[113][106]= 2;
H1[113][235]= 7;
H1[113][247]= 11;
H1[113][268]= 13;
H1[114][79]= 3;
H1[114][100]= 7;
H1[114][107]= 4;
H1[114][236]= 14;
H1[114][248]= 5;
H1[114][269]= 9;
H1[115][80]= 6;
H1[115][101]= 14;
H1[115][108]= 8;
H1[115][237]= 15;
H1[115][249]= 10;
H1[115][255]= 1;
H1[116][81]= 12;
H1[116][102]= 15;
H1[116][109]= 3;
H1[116][238]= 13;
H1[116][250]= 7;
H1[116][256]= 2;
H1[117][82]= 11;
H1[117][103]= 13;
H1[117][110]= 6;
H1[117][239]= 9;
H1[117][251]= 14;
H1[117][257]= 4;
H1[118][83]= 5;
H1[118][104]= 9;
H1[118][111]= 12;
H1[118][225]= 1;
H1[118][252]= 15;
H1[118][258]= 8;
H1[119][84]= 10;
H1[119][90]= 1;
H1[119][112]= 11;
H1[119][226]= 2;
H1[119][253]= 13;
H1[119][259]= 3;
H1[120][100]= 7;
H1[120][106]= 2;
H1[120][128]= 5;
H1[120][242]= 4;
H1[120][269]= 9;
H1[120][275]= 6;
H1[121][101]= 14;
H1[121][107]= 4;
H1[121][129]= 10;
H1[121][243]= 8;
H1[121][255]= 1;
H1[121][276]= 12;
H1[122][102]= 15;
H1[122][108]= 8;
H1[122][130]= 7;
H1[122][244]= 3;
H1[122][256]= 2;
H1[122][277]= 11;
H1[123][103]= 13;
H1[123][109]= 3;
H1[123][131]= 14;
H1[123][245]= 6;
H1[123][257]= 4;
H1[123][278]= 5;
H1[124][104]= 9;
H1[124][110]= 6;
H1[124][132]= 15;
H1[124][246]= 12;
H1[124][258]= 8;
H1[124][279]= 10;
H1[125][90]= 1;
H1[125][111]= 12;
H1[125][133]= 13;
H1[125][247]= 11;
H1[125][259]= 3;
H1[125][280]= 7;
H1[126][91]= 2;
H1[126][112]= 11;
H1[126][134]= 9;
H1[126][248]= 5;
H1[126][260]= 6;
H1[126][281]= 14;
H1[127][92]= 4;
H1[127][113]= 5;
H1[127][120]= 1;
H1[127][249]= 10;
H1[127][261]= 12;
H1[127][282]= 15;
H1[128][93]= 8;
H1[128][114]= 10;
H1[128][121]= 2;
H1[128][250]= 7;
H1[128][262]= 11;
H1[128][283]= 13;
H1[129][94]= 3;
H1[129][115]= 7;
H1[129][122]= 4;
H1[129][251]= 14;
H1[129][263]= 5;
H1[129][284]= 9;
H1[130][95]= 6;
H1[130][116]= 14;
H1[130][123]= 8;
H1[130][252]= 15;
H1[130][264]= 10;
H1[130][270]= 1;
H1[131][96]= 12;
H1[131][117]= 15;
H1[131][124]= 3;
H1[131][253]= 13;
H1[131][265]= 7;
H1[131][271]= 2;
H1[132][97]= 11;
H1[132][118]= 13;
H1[132][125]= 6;
H1[132][254]= 9;
H1[132][266]= 14;
H1[132][272]= 4;
H1[133][98]= 5;
H1[133][119]= 9;
H1[133][126]= 12;
H1[133][240]= 1;
H1[133][267]= 15;
H1[133][273]= 8;
H1[134][99]= 10;
H1[134][105]= 1;
H1[134][127]= 11;
H1[134][241]= 2;
H1[134][268]= 13;
H1[134][274]= 3;
H1[135][115]= 7;
H1[135][121]= 2;
H1[135][143]= 5;
H1[135][257]= 4;
H1[135][284]= 9;
H1[135][290]= 6;
H1[136][116]= 14;
H1[136][122]= 4;
H1[136][144]= 10;
H1[136][258]= 8;
H1[136][270]= 1;
H1[136][291]= 12;
H1[137][117]= 15;
H1[137][123]= 8;
H1[137][145]= 7;
H1[137][259]= 3;
H1[137][271]= 2;
H1[137][292]= 11;
H1[138][118]= 13;
H1[138][124]= 3;
H1[138][146]= 14;
H1[138][260]= 6;
H1[138][272]= 4;
H1[138][293]= 5;
H1[139][119]= 9;
H1[139][125]= 6;
H1[139][147]= 15;
H1[139][261]= 12;
H1[139][273]= 8;
H1[139][294]= 10;
H1[140][105]= 1;
H1[140][126]= 12;
H1[140][148]= 13;
H1[140][262]= 11;
H1[140][274]= 3;
H1[140][295]= 7;
H1[141][106]= 2;
H1[141][127]= 11;
H1[141][149]= 9;
H1[141][263]= 5;
H1[141][275]= 6;
H1[141][296]= 14;
H1[142][107]= 4;
H1[142][128]= 5;
H1[142][135]= 1;
H1[142][264]= 10;
H1[142][276]= 12;
H1[142][297]= 15;
H1[143][108]= 8;
H1[143][129]= 10;
H1[143][136]= 2;
H1[143][265]= 7;
H1[143][277]= 11;
H1[143][298]= 13;
H1[144][109]= 3;
H1[144][130]= 7;
H1[144][137]= 4;
H1[144][266]= 14;
H1[144][278]= 5;
H1[144][299]= 9;
H1[145][110]= 6;
H1[145][131]= 14;
H1[145][138]= 8;
H1[145][267]= 15;
H1[145][279]= 10;
H1[145][285]= 1;
H1[146][111]= 12;
H1[146][132]= 15;
H1[146][139]= 3;
H1[146][268]= 13;
H1[146][280]= 7;
H1[146][286]= 2;
H1[147][112]= 11;
H1[147][133]= 13;
H1[147][140]= 6;
H1[147][269]= 9;
H1[147][281]= 14;
H1[147][287]= 4;
H1[148][113]= 5;
H1[148][134]= 9;
H1[148][141]= 12;
H1[148][255]= 1;
H1[148][282]= 15;
H1[148][288]= 8;
H1[149][114]= 10;
H1[149][120]= 1;
H1[149][142]= 11;
H1[149][256]= 2;
H1[149][283]= 13;
H1[149][289]= 3;

col_row1[0][0]= 8;
col_row1[0][1]= 122;
col_row1[0][2]= 149;
col_row1[0][3]= 155;
col_row1[0][4]= 276;
col_row1[0][5]= 288;
col_row1[1][0]= 9;
col_row1[1][1]= 123;
col_row1[1][2]= 135;
col_row1[1][3]= 156;
col_row1[1][4]= 277;
col_row1[1][5]= 289;
col_row1[2][0]= 10;
col_row1[2][1]= 124;
col_row1[2][2]= 136;
col_row1[2][3]= 157;
col_row1[2][4]= 278;
col_row1[2][5]= 290;
col_row1[3][0]= 11;
col_row1[3][1]= 125;
col_row1[3][2]= 137;
col_row1[3][3]= 158;
col_row1[3][4]= 279;
col_row1[3][5]= 291;
col_row1[4][0]= 12;
col_row1[4][1]= 126;
col_row1[4][2]= 138;
col_row1[4][3]= 159;
col_row1[4][4]= 280;
col_row1[4][5]= 292;
col_row1[5][0]= 13;
col_row1[5][1]= 127;
col_row1[5][2]= 139;
col_row1[5][3]= 160;
col_row1[5][4]= 281;
col_row1[5][5]= 293;
col_row1[6][0]= 14;
col_row1[6][1]= 128;
col_row1[6][2]= 140;
col_row1[6][3]= 161;
col_row1[6][4]= 282;
col_row1[6][5]= 294;
col_row1[7][0]= 0;
col_row1[7][1]= 129;
col_row1[7][2]= 141;
col_row1[7][3]= 162;
col_row1[7][4]= 283;
col_row1[7][5]= 295;
col_row1[8][0]= 1;
col_row1[8][1]= 130;
col_row1[8][2]= 142;
col_row1[8][3]= 163;
col_row1[8][4]= 284;
col_row1[8][5]= 296;
col_row1[9][0]= 2;
col_row1[9][1]= 131;
col_row1[9][2]= 143;
col_row1[9][3]= 164;
col_row1[9][4]= 270;
col_row1[9][5]= 297;
col_row1[10][0]= 3;
col_row1[10][1]= 132;
col_row1[10][2]= 144;
col_row1[10][3]= 150;
col_row1[10][4]= 271;
col_row1[10][5]= 298;
col_row1[11][0]= 4;
col_row1[11][1]= 133;
col_row1[11][2]= 145;
col_row1[11][3]= 151;
col_row1[11][4]= 272;
col_row1[11][5]= 299;
col_row1[12][0]= 5;
col_row1[12][1]= 134;
col_row1[12][2]= 146;
col_row1[12][3]= 152;
col_row1[12][4]= 273;
col_row1[12][5]= 285;
col_row1[13][0]= 6;
col_row1[13][1]= 120;
col_row1[13][2]= 147;
col_row1[13][3]= 153;
col_row1[13][4]= 274;
col_row1[13][5]= 286;
col_row1[14][0]= 7;
col_row1[14][1]= 121;
col_row1[14][2]= 148;
col_row1[14][3]= 154;
col_row1[14][4]= 275;
col_row1[14][5]= 287;
col_row1[15][0]= 1;
col_row1[15][1]= 23;
col_row1[15][2]= 137;
col_row1[15][3]= 164;
col_row1[15][4]= 170;
col_row1[15][5]= 291;
col_row1[16][0]= 2;
col_row1[16][1]= 24;
col_row1[16][2]= 138;
col_row1[16][3]= 150;
col_row1[16][4]= 171;
col_row1[16][5]= 292;
col_row1[17][0]= 3;
col_row1[17][1]= 25;
col_row1[17][2]= 139;
col_row1[17][3]= 151;
col_row1[17][4]= 172;
col_row1[17][5]= 293;
col_row1[18][0]= 4;
col_row1[18][1]= 26;
col_row1[18][2]= 140;
col_row1[18][3]= 152;
col_row1[18][4]= 173;
col_row1[18][5]= 294;
col_row1[19][0]= 5;
col_row1[19][1]= 27;
col_row1[19][2]= 141;
col_row1[19][3]= 153;
col_row1[19][4]= 174;
col_row1[19][5]= 295;
col_row1[20][0]= 6;
col_row1[20][1]= 28;
col_row1[20][2]= 142;
col_row1[20][3]= 154;
col_row1[20][4]= 175;
col_row1[20][5]= 296;
col_row1[21][0]= 7;
col_row1[21][1]= 29;
col_row1[21][2]= 143;
col_row1[21][3]= 155;
col_row1[21][4]= 176;
col_row1[21][5]= 297;
col_row1[22][0]= 8;
col_row1[22][1]= 15;
col_row1[22][2]= 144;
col_row1[22][3]= 156;
col_row1[22][4]= 177;
col_row1[22][5]= 298;
col_row1[23][0]= 9;
col_row1[23][1]= 16;
col_row1[23][2]= 145;
col_row1[23][3]= 157;
col_row1[23][4]= 178;
col_row1[23][5]= 299;
col_row1[24][0]= 10;
col_row1[24][1]= 17;
col_row1[24][2]= 146;
col_row1[24][3]= 158;
col_row1[24][4]= 179;
col_row1[24][5]= 285;
col_row1[25][0]= 11;
col_row1[25][1]= 18;
col_row1[25][2]= 147;
col_row1[25][3]= 159;
col_row1[25][4]= 165;
col_row1[25][5]= 286;
col_row1[26][0]= 12;
col_row1[26][1]= 19;
col_row1[26][2]= 148;
col_row1[26][3]= 160;
col_row1[26][4]= 166;
col_row1[26][5]= 287;
col_row1[27][0]= 13;
col_row1[27][1]= 20;
col_row1[27][2]= 149;
col_row1[27][3]= 161;
col_row1[27][4]= 167;
col_row1[27][5]= 288;
col_row1[28][0]= 14;
col_row1[28][1]= 21;
col_row1[28][2]= 135;
col_row1[28][3]= 162;
col_row1[28][4]= 168;
col_row1[28][5]= 289;
col_row1[29][0]= 0;
col_row1[29][1]= 22;
col_row1[29][2]= 136;
col_row1[29][3]= 163;
col_row1[29][4]= 169;
col_row1[29][5]= 290;
col_row1[30][0]= 10;
col_row1[30][1]= 16;
col_row1[30][2]= 38;
col_row1[30][3]= 152;
col_row1[30][4]= 179;
col_row1[30][5]= 185;
col_row1[31][0]= 11;
col_row1[31][1]= 17;
col_row1[31][2]= 39;
col_row1[31][3]= 153;
col_row1[31][4]= 165;
col_row1[31][5]= 186;
col_row1[32][0]= 12;
col_row1[32][1]= 18;
col_row1[32][2]= 40;
col_row1[32][3]= 154;
col_row1[32][4]= 166;
col_row1[32][5]= 187;
col_row1[33][0]= 13;
col_row1[33][1]= 19;
col_row1[33][2]= 41;
col_row1[33][3]= 155;
col_row1[33][4]= 167;
col_row1[33][5]= 188;
col_row1[34][0]= 14;
col_row1[34][1]= 20;
col_row1[34][2]= 42;
col_row1[34][3]= 156;
col_row1[34][4]= 168;
col_row1[34][5]= 189;
col_row1[35][0]= 0;
col_row1[35][1]= 21;
col_row1[35][2]= 43;
col_row1[35][3]= 157;
col_row1[35][4]= 169;
col_row1[35][5]= 190;
col_row1[36][0]= 1;
col_row1[36][1]= 22;
col_row1[36][2]= 44;
col_row1[36][3]= 158;
col_row1[36][4]= 170;
col_row1[36][5]= 191;
col_row1[37][0]= 2;
col_row1[37][1]= 23;
col_row1[37][2]= 30;
col_row1[37][3]= 159;
col_row1[37][4]= 171;
col_row1[37][5]= 192;
col_row1[38][0]= 3;
col_row1[38][1]= 24;
col_row1[38][2]= 31;
col_row1[38][3]= 160;
col_row1[38][4]= 172;
col_row1[38][5]= 193;
col_row1[39][0]= 4;
col_row1[39][1]= 25;
col_row1[39][2]= 32;
col_row1[39][3]= 161;
col_row1[39][4]= 173;
col_row1[39][5]= 194;
col_row1[40][0]= 5;
col_row1[40][1]= 26;
col_row1[40][2]= 33;
col_row1[40][3]= 162;
col_row1[40][4]= 174;
col_row1[40][5]= 180;
col_row1[41][0]= 6;
col_row1[41][1]= 27;
col_row1[41][2]= 34;
col_row1[41][3]= 163;
col_row1[41][4]= 175;
col_row1[41][5]= 181;
col_row1[42][0]= 7;
col_row1[42][1]= 28;
col_row1[42][2]= 35;
col_row1[42][3]= 164;
col_row1[42][4]= 176;
col_row1[42][5]= 182;
col_row1[43][0]= 8;
col_row1[43][1]= 29;
col_row1[43][2]= 36;
col_row1[43][3]= 150;
col_row1[43][4]= 177;
col_row1[43][5]= 183;
col_row1[44][0]= 9;
col_row1[44][1]= 15;
col_row1[44][2]= 37;
col_row1[44][3]= 151;
col_row1[44][4]= 178;
col_row1[44][5]= 184;
col_row1[45][0]= 25;
col_row1[45][1]= 31;
col_row1[45][2]= 53;
col_row1[45][3]= 167;
col_row1[45][4]= 194;
col_row1[45][5]= 200;
col_row1[46][0]= 26;
col_row1[46][1]= 32;
col_row1[46][2]= 54;
col_row1[46][3]= 168;
col_row1[46][4]= 180;
col_row1[46][5]= 201;
col_row1[47][0]= 27;
col_row1[47][1]= 33;
col_row1[47][2]= 55;
col_row1[47][3]= 169;
col_row1[47][4]= 181;
col_row1[47][5]= 202;
col_row1[48][0]= 28;
col_row1[48][1]= 34;
col_row1[48][2]= 56;
col_row1[48][3]= 170;
col_row1[48][4]= 182;
col_row1[48][5]= 203;
col_row1[49][0]= 29;
col_row1[49][1]= 35;
col_row1[49][2]= 57;
col_row1[49][3]= 171;
col_row1[49][4]= 183;
col_row1[49][5]= 204;
col_row1[50][0]= 15;
col_row1[50][1]= 36;
col_row1[50][2]= 58;
col_row1[50][3]= 172;
col_row1[50][4]= 184;
col_row1[50][5]= 205;
col_row1[51][0]= 16;
col_row1[51][1]= 37;
col_row1[51][2]= 59;
col_row1[51][3]= 173;
col_row1[51][4]= 185;
col_row1[51][5]= 206;
col_row1[52][0]= 17;
col_row1[52][1]= 38;
col_row1[52][2]= 45;
col_row1[52][3]= 174;
col_row1[52][4]= 186;
col_row1[52][5]= 207;
col_row1[53][0]= 18;
col_row1[53][1]= 39;
col_row1[53][2]= 46;
col_row1[53][3]= 175;
col_row1[53][4]= 187;
col_row1[53][5]= 208;
col_row1[54][0]= 19;
col_row1[54][1]= 40;
col_row1[54][2]= 47;
col_row1[54][3]= 176;
col_row1[54][4]= 188;
col_row1[54][5]= 209;
col_row1[55][0]= 20;
col_row1[55][1]= 41;
col_row1[55][2]= 48;
col_row1[55][3]= 177;
col_row1[55][4]= 189;
col_row1[55][5]= 195;
col_row1[56][0]= 21;
col_row1[56][1]= 42;
col_row1[56][2]= 49;
col_row1[56][3]= 178;
col_row1[56][4]= 190;
col_row1[56][5]= 196;
col_row1[57][0]= 22;
col_row1[57][1]= 43;
col_row1[57][2]= 50;
col_row1[57][3]= 179;
col_row1[57][4]= 191;
col_row1[57][5]= 197;
col_row1[58][0]= 23;
col_row1[58][1]= 44;
col_row1[58][2]= 51;
col_row1[58][3]= 165;
col_row1[58][4]= 192;
col_row1[58][5]= 198;
col_row1[59][0]= 24;
col_row1[59][1]= 30;
col_row1[59][2]= 52;
col_row1[59][3]= 166;
col_row1[59][4]= 193;
col_row1[59][5]= 199;
col_row1[60][0]= 40;
col_row1[60][1]= 46;
col_row1[60][2]= 68;
col_row1[60][3]= 182;
col_row1[60][4]= 209;
col_row1[60][5]= 215;
col_row1[61][0]= 41;
col_row1[61][1]= 47;
col_row1[61][2]= 69;
col_row1[61][3]= 183;
col_row1[61][4]= 195;
col_row1[61][5]= 216;
col_row1[62][0]= 42;
col_row1[62][1]= 48;
col_row1[62][2]= 70;
col_row1[62][3]= 184;
col_row1[62][4]= 196;
col_row1[62][5]= 217;
col_row1[63][0]= 43;
col_row1[63][1]= 49;
col_row1[63][2]= 71;
col_row1[63][3]= 185;
col_row1[63][4]= 197;
col_row1[63][5]= 218;
col_row1[64][0]= 44;
col_row1[64][1]= 50;
col_row1[64][2]= 72;
col_row1[64][3]= 186;
col_row1[64][4]= 198;
col_row1[64][5]= 219;
col_row1[65][0]= 30;
col_row1[65][1]= 51;
col_row1[65][2]= 73;
col_row1[65][3]= 187;
col_row1[65][4]= 199;
col_row1[65][5]= 220;
col_row1[66][0]= 31;
col_row1[66][1]= 52;
col_row1[66][2]= 74;
col_row1[66][3]= 188;
col_row1[66][4]= 200;
col_row1[66][5]= 221;
col_row1[67][0]= 32;
col_row1[67][1]= 53;
col_row1[67][2]= 60;
col_row1[67][3]= 189;
col_row1[67][4]= 201;
col_row1[67][5]= 222;
col_row1[68][0]= 33;
col_row1[68][1]= 54;
col_row1[68][2]= 61;
col_row1[68][3]= 190;
col_row1[68][4]= 202;
col_row1[68][5]= 223;
col_row1[69][0]= 34;
col_row1[69][1]= 55;
col_row1[69][2]= 62;
col_row1[69][3]= 191;
col_row1[69][4]= 203;
col_row1[69][5]= 224;
col_row1[70][0]= 35;
col_row1[70][1]= 56;
col_row1[70][2]= 63;
col_row1[70][3]= 192;
col_row1[70][4]= 204;
col_row1[70][5]= 210;
col_row1[71][0]= 36;
col_row1[71][1]= 57;
col_row1[71][2]= 64;
col_row1[71][3]= 193;
col_row1[71][4]= 205;
col_row1[71][5]= 211;
col_row1[72][0]= 37;
col_row1[72][1]= 58;
col_row1[72][2]= 65;
col_row1[72][3]= 194;
col_row1[72][4]= 206;
col_row1[72][5]= 212;
col_row1[73][0]= 38;
col_row1[73][1]= 59;
col_row1[73][2]= 66;
col_row1[73][3]= 180;
col_row1[73][4]= 207;
col_row1[73][5]= 213;
col_row1[74][0]= 39;
col_row1[74][1]= 45;
col_row1[74][2]= 67;
col_row1[74][3]= 181;
col_row1[74][4]= 208;
col_row1[74][5]= 214;
col_row1[75][0]= 47;
col_row1[75][1]= 74;
col_row1[75][2]= 83;
col_row1[75][3]= 201;
col_row1[75][4]= 213;
col_row1[75][5]= 230;
col_row1[76][0]= 48;
col_row1[76][1]= 60;
col_row1[76][2]= 84;
col_row1[76][3]= 202;
col_row1[76][4]= 214;
col_row1[76][5]= 231;
col_row1[77][0]= 49;
col_row1[77][1]= 61;
col_row1[77][2]= 85;
col_row1[77][3]= 203;
col_row1[77][4]= 215;
col_row1[77][5]= 232;
col_row1[78][0]= 50;
col_row1[78][1]= 62;
col_row1[78][2]= 86;
col_row1[78][3]= 204;
col_row1[78][4]= 216;
col_row1[78][5]= 233;
col_row1[79][0]= 51;
col_row1[79][1]= 63;
col_row1[79][2]= 87;
col_row1[79][3]= 205;
col_row1[79][4]= 217;
col_row1[79][5]= 234;
col_row1[80][0]= 52;
col_row1[80][1]= 64;
col_row1[80][2]= 88;
col_row1[80][3]= 206;
col_row1[80][4]= 218;
col_row1[80][5]= 235;
col_row1[81][0]= 53;
col_row1[81][1]= 65;
col_row1[81][2]= 89;
col_row1[81][3]= 207;
col_row1[81][4]= 219;
col_row1[81][5]= 236;
col_row1[82][0]= 54;
col_row1[82][1]= 66;
col_row1[82][2]= 75;
col_row1[82][3]= 208;
col_row1[82][4]= 220;
col_row1[82][5]= 237;
col_row1[83][0]= 55;
col_row1[83][1]= 67;
col_row1[83][2]= 76;
col_row1[83][3]= 209;
col_row1[83][4]= 221;
col_row1[83][5]= 238;
col_row1[84][0]= 56;
col_row1[84][1]= 68;
col_row1[84][2]= 77;
col_row1[84][3]= 195;
col_row1[84][4]= 222;
col_row1[84][5]= 239;
col_row1[85][0]= 57;
col_row1[85][1]= 69;
col_row1[85][2]= 78;
col_row1[85][3]= 196;
col_row1[85][4]= 223;
col_row1[85][5]= 225;
col_row1[86][0]= 58;
col_row1[86][1]= 70;
col_row1[86][2]= 79;
col_row1[86][3]= 197;
col_row1[86][4]= 224;
col_row1[86][5]= 226;
col_row1[87][0]= 59;
col_row1[87][1]= 71;
col_row1[87][2]= 80;
col_row1[87][3]= 198;
col_row1[87][4]= 210;
col_row1[87][5]= 227;
col_row1[88][0]= 45;
col_row1[88][1]= 72;
col_row1[88][2]= 81;
col_row1[88][3]= 199;
col_row1[88][4]= 211;
col_row1[88][5]= 228;
col_row1[89][0]= 46;
col_row1[89][1]= 73;
col_row1[89][2]= 82;
col_row1[89][3]= 200;
col_row1[89][4]= 212;
col_row1[89][5]= 229;
col_row1[90][0]= 62;
col_row1[90][1]= 76;
col_row1[90][2]= 98;
col_row1[90][3]= 216;
col_row1[90][4]= 239;
col_row1[90][5]= 245;
col_row1[91][0]= 63;
col_row1[91][1]= 77;
col_row1[91][2]= 99;
col_row1[91][3]= 217;
col_row1[91][4]= 225;
col_row1[91][5]= 246;
col_row1[92][0]= 64;
col_row1[92][1]= 78;
col_row1[92][2]= 100;
col_row1[92][3]= 218;
col_row1[92][4]= 226;
col_row1[92][5]= 247;
col_row1[93][0]= 65;
col_row1[93][1]= 79;
col_row1[93][2]= 101;
col_row1[93][3]= 219;
col_row1[93][4]= 227;
col_row1[93][5]= 248;
col_row1[94][0]= 66;
col_row1[94][1]= 80;
col_row1[94][2]= 102;
col_row1[94][3]= 220;
col_row1[94][4]= 228;
col_row1[94][5]= 249;
col_row1[95][0]= 67;
col_row1[95][1]= 81;
col_row1[95][2]= 103;
col_row1[95][3]= 221;
col_row1[95][4]= 229;
col_row1[95][5]= 250;
col_row1[96][0]= 68;
col_row1[96][1]= 82;
col_row1[96][2]= 104;
col_row1[96][3]= 222;
col_row1[96][4]= 230;
col_row1[96][5]= 251;
col_row1[97][0]= 69;
col_row1[97][1]= 83;
col_row1[97][2]= 90;
col_row1[97][3]= 223;
col_row1[97][4]= 231;
col_row1[97][5]= 252;
col_row1[98][0]= 70;
col_row1[98][1]= 84;
col_row1[98][2]= 91;
col_row1[98][3]= 224;
col_row1[98][4]= 232;
col_row1[98][5]= 253;
col_row1[99][0]= 71;
col_row1[99][1]= 85;
col_row1[99][2]= 92;
col_row1[99][3]= 210;
col_row1[99][4]= 233;
col_row1[99][5]= 254;
col_row1[100][0]= 72;
col_row1[100][1]= 86;
col_row1[100][2]= 93;
col_row1[100][3]= 211;
col_row1[100][4]= 234;
col_row1[100][5]= 240;
col_row1[101][0]= 73;
col_row1[101][1]= 87;
col_row1[101][2]= 94;
col_row1[101][3]= 212;
col_row1[101][4]= 235;
col_row1[101][5]= 241;
col_row1[102][0]= 74;
col_row1[102][1]= 88;
col_row1[102][2]= 95;
col_row1[102][3]= 213;
col_row1[102][4]= 236;
col_row1[102][5]= 242;
col_row1[103][0]= 60;
col_row1[103][1]= 89;
col_row1[103][2]= 96;
col_row1[103][3]= 214;
col_row1[103][4]= 237;
col_row1[103][5]= 243;
col_row1[104][0]= 61;
col_row1[104][1]= 75;
col_row1[104][2]= 97;
col_row1[104][3]= 215;
col_row1[104][4]= 238;
col_row1[104][5]= 244;
col_row1[105][0]= 85;
col_row1[105][1]= 91;
col_row1[105][2]= 113;
col_row1[105][3]= 227;
col_row1[105][4]= 254;
col_row1[105][5]= 260;
col_row1[106][0]= 86;
col_row1[106][1]= 92;
col_row1[106][2]= 114;
col_row1[106][3]= 228;
col_row1[106][4]= 240;
col_row1[106][5]= 261;
col_row1[107][0]= 87;
col_row1[107][1]= 93;
col_row1[107][2]= 115;
col_row1[107][3]= 229;
col_row1[107][4]= 241;
col_row1[107][5]= 262;
col_row1[108][0]= 88;
col_row1[108][1]= 94;
col_row1[108][2]= 116;
col_row1[108][3]= 230;
col_row1[108][4]= 242;
col_row1[108][5]= 263;
col_row1[109][0]= 89;
col_row1[109][1]= 95;
col_row1[109][2]= 117;
col_row1[109][3]= 231;
col_row1[109][4]= 243;
col_row1[109][5]= 264;
col_row1[110][0]= 75;
col_row1[110][1]= 96;
col_row1[110][2]= 118;
col_row1[110][3]= 232;
col_row1[110][4]= 244;
col_row1[110][5]= 265;
col_row1[111][0]= 76;
col_row1[111][1]= 97;
col_row1[111][2]= 119;
col_row1[111][3]= 233;
col_row1[111][4]= 245;
col_row1[111][5]= 266;
col_row1[112][0]= 77;
col_row1[112][1]= 98;
col_row1[112][2]= 105;
col_row1[112][3]= 234;
col_row1[112][4]= 246;
col_row1[112][5]= 267;
col_row1[113][0]= 78;
col_row1[113][1]= 99;
col_row1[113][2]= 106;
col_row1[113][3]= 235;
col_row1[113][4]= 247;
col_row1[113][5]= 268;
col_row1[114][0]= 79;
col_row1[114][1]= 100;
col_row1[114][2]= 107;
col_row1[114][3]= 236;
col_row1[114][4]= 248;
col_row1[114][5]= 269;
col_row1[115][0]= 80;
col_row1[115][1]= 101;
col_row1[115][2]= 108;
col_row1[115][3]= 237;
col_row1[115][4]= 249;
col_row1[115][5]= 255;
col_row1[116][0]= 81;
col_row1[116][1]= 102;
col_row1[116][2]= 109;
col_row1[116][3]= 238;
col_row1[116][4]= 250;
col_row1[116][5]= 256;
col_row1[117][0]= 82;
col_row1[117][1]= 103;
col_row1[117][2]= 110;
col_row1[117][3]= 239;
col_row1[117][4]= 251;
col_row1[117][5]= 257;
col_row1[118][0]= 83;
col_row1[118][1]= 104;
col_row1[118][2]= 111;
col_row1[118][3]= 225;
col_row1[118][4]= 252;
col_row1[118][5]= 258;
col_row1[119][0]= 84;
col_row1[119][1]= 90;
col_row1[119][2]= 112;
col_row1[119][3]= 226;
col_row1[119][4]= 253;
col_row1[119][5]= 259;
col_row1[120][0]= 100;
col_row1[120][1]= 106;
col_row1[120][2]= 128;
col_row1[120][3]= 242;
col_row1[120][4]= 269;
col_row1[120][5]= 275;
col_row1[121][0]= 101;
col_row1[121][1]= 107;
col_row1[121][2]= 129;
col_row1[121][3]= 243;
col_row1[121][4]= 255;
col_row1[121][5]= 276;
col_row1[122][0]= 102;
col_row1[122][1]= 108;
col_row1[122][2]= 130;
col_row1[122][3]= 244;
col_row1[122][4]= 256;
col_row1[122][5]= 277;
col_row1[123][0]= 103;
col_row1[123][1]= 109;
col_row1[123][2]= 131;
col_row1[123][3]= 245;
col_row1[123][4]= 257;
col_row1[123][5]= 278;
col_row1[124][0]= 104;
col_row1[124][1]= 110;
col_row1[124][2]= 132;
col_row1[124][3]= 246;
col_row1[124][4]= 258;
col_row1[124][5]= 279;
col_row1[125][0]= 90;
col_row1[125][1]= 111;
col_row1[125][2]= 133;
col_row1[125][3]= 247;
col_row1[125][4]= 259;
col_row1[125][5]= 280;
col_row1[126][0]= 91;
col_row1[126][1]= 112;
col_row1[126][2]= 134;
col_row1[126][3]= 248;
col_row1[126][4]= 260;
col_row1[126][5]= 281;
col_row1[127][0]= 92;
col_row1[127][1]= 113;
col_row1[127][2]= 120;
col_row1[127][3]= 249;
col_row1[127][4]= 261;
col_row1[127][5]= 282;
col_row1[128][0]= 93;
col_row1[128][1]= 114;
col_row1[128][2]= 121;
col_row1[128][3]= 250;
col_row1[128][4]= 262;
col_row1[128][5]= 283;
col_row1[129][0]= 94;
col_row1[129][1]= 115;
col_row1[129][2]= 122;
col_row1[129][3]= 251;
col_row1[129][4]= 263;
col_row1[129][5]= 284;
col_row1[130][0]= 95;
col_row1[130][1]= 116;
col_row1[130][2]= 123;
col_row1[130][3]= 252;
col_row1[130][4]= 264;
col_row1[130][5]= 270;
col_row1[131][0]= 96;
col_row1[131][1]= 117;
col_row1[131][2]= 124;
col_row1[131][3]= 253;
col_row1[131][4]= 265;
col_row1[131][5]= 271;
col_row1[132][0]= 97;
col_row1[132][1]= 118;
col_row1[132][2]= 125;
col_row1[132][3]= 254;
col_row1[132][4]= 266;
col_row1[132][5]= 272;
col_row1[133][0]= 98;
col_row1[133][1]= 119;
col_row1[133][2]= 126;
col_row1[133][3]= 240;
col_row1[133][4]= 267;
col_row1[133][5]= 273;
col_row1[134][0]= 99;
col_row1[134][1]= 105;
col_row1[134][2]= 127;
col_row1[134][3]= 241;
col_row1[134][4]= 268;
col_row1[134][5]= 274;
col_row1[135][0]= 115;
col_row1[135][1]= 121;
col_row1[135][2]= 143;
col_row1[135][3]= 257;
col_row1[135][4]= 284;
col_row1[135][5]= 290;
col_row1[136][0]= 116;
col_row1[136][1]= 122;
col_row1[136][2]= 144;
col_row1[136][3]= 258;
col_row1[136][4]= 270;
col_row1[136][5]= 291;
col_row1[137][0]= 117;
col_row1[137][1]= 123;
col_row1[137][2]= 145;
col_row1[137][3]= 259;
col_row1[137][4]= 271;
col_row1[137][5]= 292;
col_row1[138][0]= 118;
col_row1[138][1]= 124;
col_row1[138][2]= 146;
col_row1[138][3]= 260;
col_row1[138][4]= 272;
col_row1[138][5]= 293;
col_row1[139][0]= 119;
col_row1[139][1]= 125;
col_row1[139][2]= 147;
col_row1[139][3]= 261;
col_row1[139][4]= 273;
col_row1[139][5]= 294;
col_row1[140][0]= 105;
col_row1[140][1]= 126;
col_row1[140][2]= 148;
col_row1[140][3]= 262;
col_row1[140][4]= 274;
col_row1[140][5]= 295;
col_row1[141][0]= 106;
col_row1[141][1]= 127;
col_row1[141][2]= 149;
col_row1[141][3]= 263;
col_row1[141][4]= 275;
col_row1[141][5]= 296;
col_row1[142][0]= 107;
col_row1[142][1]= 128;
col_row1[142][2]= 135;
col_row1[142][3]= 264;
col_row1[142][4]= 276;
col_row1[142][5]= 297;
col_row1[143][0]= 108;
col_row1[143][1]= 129;
col_row1[143][2]= 136;
col_row1[143][3]= 265;
col_row1[143][4]= 277;
col_row1[143][5]= 298;
col_row1[144][0]= 109;
col_row1[144][1]= 130;
col_row1[144][2]= 137;
col_row1[144][3]= 266;
col_row1[144][4]= 278;
col_row1[144][5]= 299;
col_row1[145][0]= 110;
col_row1[145][1]= 131;
col_row1[145][2]= 138;
col_row1[145][3]= 267;
col_row1[145][4]= 279;
col_row1[145][5]= 285;
col_row1[146][0]= 111;
col_row1[146][1]= 132;
col_row1[146][2]= 139;
col_row1[146][3]= 268;
col_row1[146][4]= 280;
col_row1[146][5]= 286;
col_row1[147][0]= 112;
col_row1[147][1]= 133;
col_row1[147][2]= 140;
col_row1[147][3]= 269;
col_row1[147][4]= 281;
col_row1[147][5]= 287;
col_row1[148][0]= 113;
col_row1[148][1]= 134;
col_row1[148][2]= 141;
col_row1[148][3]= 255;
col_row1[148][4]= 282;
col_row1[148][5]= 288;
col_row1[149][0]= 114;
col_row1[149][1]= 120;
col_row1[149][2]= 142;
col_row1[149][3]= 256;
col_row1[149][4]= 283;
col_row1[149][5]= 289;
row_col1[0][0] =7;
row_col1[1][0] =29;
row_col1[2][0] =35;
row_col1[0][1] =8;
row_col1[1][1] =15;
row_col1[2][1] =36;
row_col1[0][2] =9;
row_col1[1][2] =16;
row_col1[2][2] =37;
row_col1[0][3] =10;
row_col1[1][3] =17;
row_col1[2][3] =38;
row_col1[0][4] =11;
row_col1[1][4] =18;
row_col1[2][4] =39;
row_col1[0][5] =12;
row_col1[1][5] =19;
row_col1[2][5] =40;
row_col1[0][6] =13;
row_col1[1][6] =20;
row_col1[2][6] =41;
row_col1[0][7] =14;
row_col1[1][7] =21;
row_col1[2][7] =42;
row_col1[0][8] =0;
row_col1[1][8] =22;
row_col1[2][8] =43;
row_col1[0][9] =1;
row_col1[1][9] =23;
row_col1[2][9] =44;
row_col1[0][10] =2;
row_col1[1][10] =24;
row_col1[2][10] =30;
row_col1[0][11] =3;
row_col1[1][11] =25;
row_col1[2][11] =31;
row_col1[0][12] =4;
row_col1[1][12] =26;
row_col1[2][12] =32;
row_col1[0][13] =5;
row_col1[1][13] =27;
row_col1[2][13] =33;
row_col1[0][14] =6;
row_col1[1][14] =28;
row_col1[2][14] =34;
row_col1[0][15] =22;
row_col1[1][15] =44;
row_col1[2][15] =50;
row_col1[0][16] =23;
row_col1[1][16] =30;
row_col1[2][16] =51;
row_col1[0][17] =24;
row_col1[1][17] =31;
row_col1[2][17] =52;
row_col1[0][18] =25;
row_col1[1][18] =32;
row_col1[2][18] =53;
row_col1[0][19] =26;
row_col1[1][19] =33;
row_col1[2][19] =54;
row_col1[0][20] =27;
row_col1[1][20] =34;
row_col1[2][20] =55;
row_col1[0][21] =28;
row_col1[1][21] =35;
row_col1[2][21] =56;
row_col1[0][22] =29;
row_col1[1][22] =36;
row_col1[2][22] =57;
row_col1[0][23] =15;
row_col1[1][23] =37;
row_col1[2][23] =58;
row_col1[0][24] =16;
row_col1[1][24] =38;
row_col1[2][24] =59;
row_col1[0][25] =17;
row_col1[1][25] =39;
row_col1[2][25] =45;
row_col1[0][26] =18;
row_col1[1][26] =40;
row_col1[2][26] =46;
row_col1[0][27] =19;
row_col1[1][27] =41;
row_col1[2][27] =47;
row_col1[0][28] =20;
row_col1[1][28] =42;
row_col1[2][28] =48;
row_col1[0][29] =21;
row_col1[1][29] =43;
row_col1[2][29] =49;
row_col1[0][30] =37;
row_col1[1][30] =59;
row_col1[2][30] =65;
row_col1[0][31] =38;
row_col1[1][31] =45;
row_col1[2][31] =66;
row_col1[0][32] =39;
row_col1[1][32] =46;
row_col1[2][32] =67;
row_col1[0][33] =40;
row_col1[1][33] =47;
row_col1[2][33] =68;
row_col1[0][34] =41;
row_col1[1][34] =48;
row_col1[2][34] =69;
row_col1[0][35] =42;
row_col1[1][35] =49;
row_col1[2][35] =70;
row_col1[0][36] =43;
row_col1[1][36] =50;
row_col1[2][36] =71;
row_col1[0][37] =44;
row_col1[1][37] =51;
row_col1[2][37] =72;
row_col1[0][38] =30;
row_col1[1][38] =52;
row_col1[2][38] =73;
row_col1[0][39] =31;
row_col1[1][39] =53;
row_col1[2][39] =74;
row_col1[0][40] =32;
row_col1[1][40] =54;
row_col1[2][40] =60;
row_col1[0][41] =33;
row_col1[1][41] =55;
row_col1[2][41] =61;
row_col1[0][42] =34;
row_col1[1][42] =56;
row_col1[2][42] =62;
row_col1[0][43] =35;
row_col1[1][43] =57;
row_col1[2][43] =63;
row_col1[0][44] =36;
row_col1[1][44] =58;
row_col1[2][44] =64;
row_col1[0][45] =52;
row_col1[1][45] =74;
row_col1[2][45] =88;
row_col1[0][46] =53;
row_col1[1][46] =60;
row_col1[2][46] =89;
row_col1[0][47] =54;
row_col1[1][47] =61;
row_col1[2][47] =75;
row_col1[0][48] =55;
row_col1[1][48] =62;
row_col1[2][48] =76;
row_col1[0][49] =56;
row_col1[1][49] =63;
row_col1[2][49] =77;
row_col1[0][50] =57;
row_col1[1][50] =64;
row_col1[2][50] =78;
row_col1[0][51] =58;
row_col1[1][51] =65;
row_col1[2][51] =79;
row_col1[0][52] =59;
row_col1[1][52] =66;
row_col1[2][52] =80;
row_col1[0][53] =45;
row_col1[1][53] =67;
row_col1[2][53] =81;
row_col1[0][54] =46;
row_col1[1][54] =68;
row_col1[2][54] =82;
row_col1[0][55] =47;
row_col1[1][55] =69;
row_col1[2][55] =83;
row_col1[0][56] =48;
row_col1[1][56] =70;
row_col1[2][56] =84;
row_col1[0][57] =49;
row_col1[1][57] =71;
row_col1[2][57] =85;
row_col1[0][58] =50;
row_col1[1][58] =72;
row_col1[2][58] =86;
row_col1[0][59] =51;
row_col1[1][59] =73;
row_col1[2][59] =87;
row_col1[0][60] =67;
row_col1[1][60] =76;
row_col1[2][60] =103;
row_col1[0][61] =68;
row_col1[1][61] =77;
row_col1[2][61] =104;
row_col1[0][62] =69;
row_col1[1][62] =78;
row_col1[2][62] =90;
row_col1[0][63] =70;
row_col1[1][63] =79;
row_col1[2][63] =91;
row_col1[0][64] =71;
row_col1[1][64] =80;
row_col1[2][64] =92;
row_col1[0][65] =72;
row_col1[1][65] =81;
row_col1[2][65] =93;
row_col1[0][66] =73;
row_col1[1][66] =82;
row_col1[2][66] =94;
row_col1[0][67] =74;
row_col1[1][67] =83;
row_col1[2][67] =95;
row_col1[0][68] =60;
row_col1[1][68] =84;
row_col1[2][68] =96;
row_col1[0][69] =61;
row_col1[1][69] =85;
row_col1[2][69] =97;
row_col1[0][70] =62;
row_col1[1][70] =86;
row_col1[2][70] =98;
row_col1[0][71] =63;
row_col1[1][71] =87;
row_col1[2][71] =99;
row_col1[0][72] =64;
row_col1[1][72] =88;
row_col1[2][72] =100;
row_col1[0][73] =65;
row_col1[1][73] =89;
row_col1[2][73] =101;
row_col1[0][74] =66;
row_col1[1][74] =75;
row_col1[2][74] =102;
row_col1[0][75] =82;
row_col1[1][75] =104;
row_col1[2][75] =110;
row_col1[0][76] =83;
row_col1[1][76] =90;
row_col1[2][76] =111;
row_col1[0][77] =84;
row_col1[1][77] =91;
row_col1[2][77] =112;
row_col1[0][78] =85;
row_col1[1][78] =92;
row_col1[2][78] =113;
row_col1[0][79] =86;
row_col1[1][79] =93;
row_col1[2][79] =114;
row_col1[0][80] =87;
row_col1[1][80] =94;
row_col1[2][80] =115;
row_col1[0][81] =88;
row_col1[1][81] =95;
row_col1[2][81] =116;
row_col1[0][82] =89;
row_col1[1][82] =96;
row_col1[2][82] =117;
row_col1[0][83] =75;
row_col1[1][83] =97;
row_col1[2][83] =118;
row_col1[0][84] =76;
row_col1[1][84] =98;
row_col1[2][84] =119;
row_col1[0][85] =77;
row_col1[1][85] =99;
row_col1[2][85] =105;
row_col1[0][86] =78;
row_col1[1][86] =100;
row_col1[2][86] =106;
row_col1[0][87] =79;
row_col1[1][87] =101;
row_col1[2][87] =107;
row_col1[0][88] =80;
row_col1[1][88] =102;
row_col1[2][88] =108;
row_col1[0][89] =81;
row_col1[1][89] =103;
row_col1[2][89] =109;
row_col1[0][90] =97;
row_col1[1][90] =119;
row_col1[2][90] =125;
row_col1[0][91] =98;
row_col1[1][91] =105;
row_col1[2][91] =126;
row_col1[0][92] =99;
row_col1[1][92] =106;
row_col1[2][92] =127;
row_col1[0][93] =100;
row_col1[1][93] =107;
row_col1[2][93] =128;
row_col1[0][94] =101;
row_col1[1][94] =108;
row_col1[2][94] =129;
row_col1[0][95] =102;
row_col1[1][95] =109;
row_col1[2][95] =130;
row_col1[0][96] =103;
row_col1[1][96] =110;
row_col1[2][96] =131;
row_col1[0][97] =104;
row_col1[1][97] =111;
row_col1[2][97] =132;
row_col1[0][98] =90;
row_col1[1][98] =112;
row_col1[2][98] =133;
row_col1[0][99] =91;
row_col1[1][99] =113;
row_col1[2][99] =134;
row_col1[0][100] =92;
row_col1[1][100] =114;
row_col1[2][100] =120;
row_col1[0][101] =93;
row_col1[1][101] =115;
row_col1[2][101] =121;
row_col1[0][102] =94;
row_col1[1][102] =116;
row_col1[2][102] =122;
row_col1[0][103] =95;
row_col1[1][103] =117;
row_col1[2][103] =123;
row_col1[0][104] =96;
row_col1[1][104] =118;
row_col1[2][104] =124;
row_col1[0][105] =112;
row_col1[1][105] =134;
row_col1[2][105] =140;
row_col1[0][106] =113;
row_col1[1][106] =120;
row_col1[2][106] =141;
row_col1[0][107] =114;
row_col1[1][107] =121;
row_col1[2][107] =142;
row_col1[0][108] =115;
row_col1[1][108] =122;
row_col1[2][108] =143;
row_col1[0][109] =116;
row_col1[1][109] =123;
row_col1[2][109] =144;
row_col1[0][110] =117;
row_col1[1][110] =124;
row_col1[2][110] =145;
row_col1[0][111] =118;
row_col1[1][111] =125;
row_col1[2][111] =146;
row_col1[0][112] =119;
row_col1[1][112] =126;
row_col1[2][112] =147;
row_col1[0][113] =105;
row_col1[1][113] =127;
row_col1[2][113] =148;
row_col1[0][114] =106;
row_col1[1][114] =128;
row_col1[2][114] =149;
row_col1[0][115] =107;
row_col1[1][115] =129;
row_col1[2][115] =135;
row_col1[0][116] =108;
row_col1[1][116] =130;
row_col1[2][116] =136;
row_col1[0][117] =109;
row_col1[1][117] =131;
row_col1[2][117] =137;
row_col1[0][118] =110;
row_col1[1][118] =132;
row_col1[2][118] =138;
row_col1[0][119] =111;
row_col1[1][119] =133;
row_col1[2][119] =139;
row_col1[0][120] =13;
row_col1[1][120] =127;
row_col1[2][120] =149;
row_col1[0][121] =14;
row_col1[1][121] =128;
row_col1[2][121] =135;
row_col1[0][122] =0;
row_col1[1][122] =129;
row_col1[2][122] =136;
row_col1[0][123] =1;
row_col1[1][123] =130;
row_col1[2][123] =137;
row_col1[0][124] =2;
row_col1[1][124] =131;
row_col1[2][124] =138;
row_col1[0][125] =3;
row_col1[1][125] =132;
row_col1[2][125] =139;
row_col1[0][126] =4;
row_col1[1][126] =133;
row_col1[2][126] =140;
row_col1[0][127] =5;
row_col1[1][127] =134;
row_col1[2][127] =141;
row_col1[0][128] =6;
row_col1[1][128] =120;
row_col1[2][128] =142;
row_col1[0][129] =7;
row_col1[1][129] =121;
row_col1[2][129] =143;
row_col1[0][130] =8;
row_col1[1][130] =122;
row_col1[2][130] =144;
row_col1[0][131] =9;
row_col1[1][131] =123;
row_col1[2][131] =145;
row_col1[0][132] =10;
row_col1[1][132] =124;
row_col1[2][132] =146;
row_col1[0][133] =11;
row_col1[1][133] =125;
row_col1[2][133] =147;
row_col1[0][134] =12;
row_col1[1][134] =126;
row_col1[2][134] =148;
row_col1[0][135] =1;
row_col1[1][135] =28;
row_col1[2][135] =142;
row_col1[0][136] =2;
row_col1[1][136] =29;
row_col1[2][136] =143;
row_col1[0][137] =3;
row_col1[1][137] =15;
row_col1[2][137] =144;
row_col1[0][138] =4;
row_col1[1][138] =16;
row_col1[2][138] =145;
row_col1[0][139] =5;
row_col1[1][139] =17;
row_col1[2][139] =146;
row_col1[0][140] =6;
row_col1[1][140] =18;
row_col1[2][140] =147;
row_col1[0][141] =7;
row_col1[1][141] =19;
row_col1[2][141] =148;
row_col1[0][142] =8;
row_col1[1][142] =20;
row_col1[2][142] =149;
row_col1[0][143] =9;
row_col1[1][143] =21;
row_col1[2][143] =135;
row_col1[0][144] =10;
row_col1[1][144] =22;
row_col1[2][144] =136;
row_col1[0][145] =11;
row_col1[1][145] =23;
row_col1[2][145] =137;
row_col1[0][146] =12;
row_col1[1][146] =24;
row_col1[2][146] =138;
row_col1[0][147] =13;
row_col1[1][147] =25;
row_col1[2][147] =139;
row_col1[0][148] =14;
row_col1[1][148] =26;
row_col1[2][148] =140;
row_col1[0][149] =0;
row_col1[1][149] =27;
row_col1[2][149] =141;
row_col1[0][150] =10;
row_col1[1][150] =16;
row_col1[2][150] =43;
row_col1[0][151] =11;
row_col1[1][151] =17;
row_col1[2][151] =44;
row_col1[0][152] =12;
row_col1[1][152] =18;
row_col1[2][152] =30;
row_col1[0][153] =13;
row_col1[1][153] =19;
row_col1[2][153] =31;
row_col1[0][154] =14;
row_col1[1][154] =20;
row_col1[2][154] =32;
row_col1[0][155] =0;
row_col1[1][155] =21;
row_col1[2][155] =33;
row_col1[0][156] =1;
row_col1[1][156] =22;
row_col1[2][156] =34;
row_col1[0][157] =2;
row_col1[1][157] =23;
row_col1[2][157] =35;
row_col1[0][158] =3;
row_col1[1][158] =24;
row_col1[2][158] =36;
row_col1[0][159] =4;
row_col1[1][159] =25;
row_col1[2][159] =37;
row_col1[0][160] =5;
row_col1[1][160] =26;
row_col1[2][160] =38;
row_col1[0][161] =6;
row_col1[1][161] =27;
row_col1[2][161] =39;
row_col1[0][162] =7;
row_col1[1][162] =28;
row_col1[2][162] =40;
row_col1[0][163] =8;
row_col1[1][163] =29;
row_col1[2][163] =41;
row_col1[0][164] =9;
row_col1[1][164] =15;
row_col1[2][164] =42;
row_col1[0][165] =25;
row_col1[1][165] =31;
row_col1[2][165] =58;
row_col1[0][166] =26;
row_col1[1][166] =32;
row_col1[2][166] =59;
row_col1[0][167] =27;
row_col1[1][167] =33;
row_col1[2][167] =45;
row_col1[0][168] =28;
row_col1[1][168] =34;
row_col1[2][168] =46;
row_col1[0][169] =29;
row_col1[1][169] =35;
row_col1[2][169] =47;
row_col1[0][170] =15;
row_col1[1][170] =36;
row_col1[2][170] =48;
row_col1[0][171] =16;
row_col1[1][171] =37;
row_col1[2][171] =49;
row_col1[0][172] =17;
row_col1[1][172] =38;
row_col1[2][172] =50;
row_col1[0][173] =18;
row_col1[1][173] =39;
row_col1[2][173] =51;
row_col1[0][174] =19;
row_col1[1][174] =40;
row_col1[2][174] =52;
row_col1[0][175] =20;
row_col1[1][175] =41;
row_col1[2][175] =53;
row_col1[0][176] =21;
row_col1[1][176] =42;
row_col1[2][176] =54;
row_col1[0][177] =22;
row_col1[1][177] =43;
row_col1[2][177] =55;
row_col1[0][178] =23;
row_col1[1][178] =44;
row_col1[2][178] =56;
row_col1[0][179] =24;
row_col1[1][179] =30;
row_col1[2][179] =57;
row_col1[0][180] =40;
row_col1[1][180] =46;
row_col1[2][180] =73;
row_col1[0][181] =41;
row_col1[1][181] =47;
row_col1[2][181] =74;
row_col1[0][182] =42;
row_col1[1][182] =48;
row_col1[2][182] =60;
row_col1[0][183] =43;
row_col1[1][183] =49;
row_col1[2][183] =61;
row_col1[0][184] =44;
row_col1[1][184] =50;
row_col1[2][184] =62;
row_col1[0][185] =30;
row_col1[1][185] =51;
row_col1[2][185] =63;
row_col1[0][186] =31;
row_col1[1][186] =52;
row_col1[2][186] =64;
row_col1[0][187] =32;
row_col1[1][187] =53;
row_col1[2][187] =65;
row_col1[0][188] =33;
row_col1[1][188] =54;
row_col1[2][188] =66;
row_col1[0][189] =34;
row_col1[1][189] =55;
row_col1[2][189] =67;
row_col1[0][190] =35;
row_col1[1][190] =56;
row_col1[2][190] =68;
row_col1[0][191] =36;
row_col1[1][191] =57;
row_col1[2][191] =69;
row_col1[0][192] =37;
row_col1[1][192] =58;
row_col1[2][192] =70;
row_col1[0][193] =38;
row_col1[1][193] =59;
row_col1[2][193] =71;
row_col1[0][194] =39;
row_col1[1][194] =45;
row_col1[2][194] =72;
row_col1[0][195] =55;
row_col1[1][195] =61;
row_col1[2][195] =84;
row_col1[0][196] =56;
row_col1[1][196] =62;
row_col1[2][196] =85;
row_col1[0][197] =57;
row_col1[1][197] =63;
row_col1[2][197] =86;
row_col1[0][198] =58;
row_col1[1][198] =64;
row_col1[2][198] =87;
row_col1[0][199] =59;
row_col1[1][199] =65;
row_col1[2][199] =88;
row_col1[0][200] =45;
row_col1[1][200] =66;
row_col1[2][200] =89;
row_col1[0][201] =46;
row_col1[1][201] =67;
row_col1[2][201] =75;
row_col1[0][202] =47;
row_col1[1][202] =68;
row_col1[2][202] =76;
row_col1[0][203] =48;
row_col1[1][203] =69;
row_col1[2][203] =77;
row_col1[0][204] =49;
row_col1[1][204] =70;
row_col1[2][204] =78;
row_col1[0][205] =50;
row_col1[1][205] =71;
row_col1[2][205] =79;
row_col1[0][206] =51;
row_col1[1][206] =72;
row_col1[2][206] =80;
row_col1[0][207] =52;
row_col1[1][207] =73;
row_col1[2][207] =81;
row_col1[0][208] =53;
row_col1[1][208] =74;
row_col1[2][208] =82;
row_col1[0][209] =54;
row_col1[1][209] =60;
row_col1[2][209] =83;
row_col1[0][210] =70;
row_col1[1][210] =87;
row_col1[2][210] =99;
row_col1[0][211] =71;
row_col1[1][211] =88;
row_col1[2][211] =100;
row_col1[0][212] =72;
row_col1[1][212] =89;
row_col1[2][212] =101;
row_col1[0][213] =73;
row_col1[1][213] =75;
row_col1[2][213] =102;
row_col1[0][214] =74;
row_col1[1][214] =76;
row_col1[2][214] =103;
row_col1[0][215] =60;
row_col1[1][215] =77;
row_col1[2][215] =104;
row_col1[0][216] =61;
row_col1[1][216] =78;
row_col1[2][216] =90;
row_col1[0][217] =62;
row_col1[1][217] =79;
row_col1[2][217] =91;
row_col1[0][218] =63;
row_col1[1][218] =80;
row_col1[2][218] =92;
row_col1[0][219] =64;
row_col1[1][219] =81;
row_col1[2][219] =93;
row_col1[0][220] =65;
row_col1[1][220] =82;
row_col1[2][220] =94;
row_col1[0][221] =66;
row_col1[1][221] =83;
row_col1[2][221] =95;
row_col1[0][222] =67;
row_col1[1][222] =84;
row_col1[2][222] =96;
row_col1[0][223] =68;
row_col1[1][223] =85;
row_col1[2][223] =97;
row_col1[0][224] =69;
row_col1[1][224] =86;
row_col1[2][224] =98;
row_col1[0][225] =85;
row_col1[1][225] =91;
row_col1[2][225] =118;
row_col1[0][226] =86;
row_col1[1][226] =92;
row_col1[2][226] =119;
row_col1[0][227] =87;
row_col1[1][227] =93;
row_col1[2][227] =105;
row_col1[0][228] =88;
row_col1[1][228] =94;
row_col1[2][228] =106;
row_col1[0][229] =89;
row_col1[1][229] =95;
row_col1[2][229] =107;
row_col1[0][230] =75;
row_col1[1][230] =96;
row_col1[2][230] =108;
row_col1[0][231] =76;
row_col1[1][231] =97;
row_col1[2][231] =109;
row_col1[0][232] =77;
row_col1[1][232] =98;
row_col1[2][232] =110;
row_col1[0][233] =78;
row_col1[1][233] =99;
row_col1[2][233] =111;
row_col1[0][234] =79;
row_col1[1][234] =100;
row_col1[2][234] =112;
row_col1[0][235] =80;
row_col1[1][235] =101;
row_col1[2][235] =113;
row_col1[0][236] =81;
row_col1[1][236] =102;
row_col1[2][236] =114;
row_col1[0][237] =82;
row_col1[1][237] =103;
row_col1[2][237] =115;
row_col1[0][238] =83;
row_col1[1][238] =104;
row_col1[2][238] =116;
row_col1[0][239] =84;
row_col1[1][239] =90;
row_col1[2][239] =117;
row_col1[0][240] =100;
row_col1[1][240] =106;
row_col1[2][240] =133;
row_col1[0][241] =101;
row_col1[1][241] =107;
row_col1[2][241] =134;
row_col1[0][242] =102;
row_col1[1][242] =108;
row_col1[2][242] =120;
row_col1[0][243] =103;
row_col1[1][243] =109;
row_col1[2][243] =121;
row_col1[0][244] =104;
row_col1[1][244] =110;
row_col1[2][244] =122;
row_col1[0][245] =90;
row_col1[1][245] =111;
row_col1[2][245] =123;
row_col1[0][246] =91;
row_col1[1][246] =112;
row_col1[2][246] =124;
row_col1[0][247] =92;
row_col1[1][247] =113;
row_col1[2][247] =125;
row_col1[0][248] =93;
row_col1[1][248] =114;
row_col1[2][248] =126;
row_col1[0][249] =94;
row_col1[1][249] =115;
row_col1[2][249] =127;
row_col1[0][250] =95;
row_col1[1][250] =116;
row_col1[2][250] =128;
row_col1[0][251] =96;
row_col1[1][251] =117;
row_col1[2][251] =129;
row_col1[0][252] =97;
row_col1[1][252] =118;
row_col1[2][252] =130;
row_col1[0][253] =98;
row_col1[1][253] =119;
row_col1[2][253] =131;
row_col1[0][254] =99;
row_col1[1][254] =105;
row_col1[2][254] =132;
row_col1[0][255] =115;
row_col1[1][255] =121;
row_col1[2][255] =148;
row_col1[0][256] =116;
row_col1[1][256] =122;
row_col1[2][256] =149;
row_col1[0][257] =117;
row_col1[1][257] =123;
row_col1[2][257] =135;
row_col1[0][258] =118;
row_col1[1][258] =124;
row_col1[2][258] =136;
row_col1[0][259] =119;
row_col1[1][259] =125;
row_col1[2][259] =137;
row_col1[0][260] =105;
row_col1[1][260] =126;
row_col1[2][260] =138;
row_col1[0][261] =106;
row_col1[1][261] =127;
row_col1[2][261] =139;
row_col1[0][262] =107;
row_col1[1][262] =128;
row_col1[2][262] =140;
row_col1[0][263] =108;
row_col1[1][263] =129;
row_col1[2][263] =141;
row_col1[0][264] =109;
row_col1[1][264] =130;
row_col1[2][264] =142;
row_col1[0][265] =110;
row_col1[1][265] =131;
row_col1[2][265] =143;
row_col1[0][266] =111;
row_col1[1][266] =132;
row_col1[2][266] =144;
row_col1[0][267] =112;
row_col1[1][267] =133;
row_col1[2][267] =145;
row_col1[0][268] =113;
row_col1[1][268] =134;
row_col1[2][268] =146;
row_col1[0][269] =114;
row_col1[1][269] =120;
row_col1[2][269] =147;
row_col1[0][270] =9;
row_col1[1][270] =130;
row_col1[2][270] =136;
row_col1[0][271] =10;
row_col1[1][271] =131;
row_col1[2][271] =137;
row_col1[0][272] =11;
row_col1[1][272] =132;
row_col1[2][272] =138;
row_col1[0][273] =12;
row_col1[1][273] =133;
row_col1[2][273] =139;
row_col1[0][274] =13;
row_col1[1][274] =134;
row_col1[2][274] =140;
row_col1[0][275] =14;
row_col1[1][275] =120;
row_col1[2][275] =141;
row_col1[0][276] =0;
row_col1[1][276] =121;
row_col1[2][276] =142;
row_col1[0][277] =1;
row_col1[1][277] =122;
row_col1[2][277] =143;
row_col1[0][278] =2;
row_col1[1][278] =123;
row_col1[2][278] =144;
row_col1[0][279] =3;
row_col1[1][279] =124;
row_col1[2][279] =145;
row_col1[0][280] =4;
row_col1[1][280] =125;
row_col1[2][280] =146;
row_col1[0][281] =5;
row_col1[1][281] =126;
row_col1[2][281] =147;
row_col1[0][282] =6;
row_col1[1][282] =127;
row_col1[2][282] =148;
row_col1[0][283] =7;
row_col1[1][283] =128;
row_col1[2][283] =149;
row_col1[0][284] =8;
row_col1[1][284] =129;
row_col1[2][284] =135;
row_col1[0][285] =12;
row_col1[1][285] =24;
row_col1[2][285] =145;
row_col1[0][286] =13;
row_col1[1][286] =25;
row_col1[2][286] =146;
row_col1[0][287] =14;
row_col1[1][287] =26;
row_col1[2][287] =147;
row_col1[0][288] =0;
row_col1[1][288] =27;
row_col1[2][288] =148;
row_col1[0][289] =1;
row_col1[1][289] =28;
row_col1[2][289] =149;
row_col1[0][290] =2;
row_col1[1][290] =29;
row_col1[2][290] =135;
row_col1[0][291] =3;
row_col1[1][291] =15;
row_col1[2][291] =136;
row_col1[0][292] =4;
row_col1[1][292] =16;
row_col1[2][292] =137;
row_col1[0][293] =5;
row_col1[1][293] =17;
row_col1[2][293] =138;
row_col1[0][294] =6;
row_col1[1][294] =18;
row_col1[2][294] =139;
row_col1[0][295] =7;
row_col1[1][295] =19;
row_col1[2][295] =140;
row_col1[0][296] =8;
row_col1[1][296] =20;
row_col1[2][296] =141;
row_col1[0][297] =9;
row_col1[1][297] =21;
row_col1[2][297] =142;
row_col1[0][298] =10;
row_col1[1][298] =22;
row_col1[2][298] =143;
row_col1[0][299] =11;
row_col1[1][299] =23;
row_col1[2][299] =144;
*/

unsigned char codeword_sym[V];
#pragma HLS ARRAY_PARTITION variable=codeword_sym complete dim=1
	unsigned char B[Q][Y];
	unsigned char F[Q][Y];


	for(i = 0; i < Q; i++){
#pragma HLS unroll
		for(j = 0; j < Y; j++){

#pragma HLS unroll

#pragma HLS ARRAY_PARTITION variable=F complete dim=1
#pragma HLS ARRAY_PARTITION variable=B complete dim=1

			F[i][j] = B[i][j] = 0.0;
		}
	}

	for(i = 0; i < Q; i++){
#pragma HLS unroll
		for(j = 0; j < V; j++){
#pragma HLS unroll
#pragma HLS ARRAY_PARTITION variable=GAMMAn_post complete dim=2
			GAMMAn_post[i][j] = 0.0;
		}
	}


	minmax_label1:for(row = 0; row < U; row++)// initialize variable node message ALPHA with channel info GAMMA
	{
//#pragma HLS pipeline II=2
		col = col_row1[row][0];
		minmax_label23:for(index = 0; index <6 ; index++)
		{
#pragma HLS unroll
			col = col_row1[row][index];
			for(a = 0; a < Q; a++){
#pragma HLS unroll
				ALPHAmn_1[row][col][a] = GAMMAn_[a][col];
			//	printf(" ALPHAMN[%d][%d][%d] %hhu \n ",row, col,a,ALPHAmn_1[row][col][a]);
			}
		}
	}
/*

	criticalpath1:
	for(col = 0; col < V; col ++){// tentative decoding
#pragma HLS pipeline II=3
		a = 0;
		for(i = 0; i < Q; i++){
//#pragma HLS pipeline II=1
			if(GAMMAn_[i][col] < GAMMAn_[a][col])
			{
				a = i;
			}
		}
		codeword_sym[col] = a;
		//printf("codeword symbol %hhu\n",codeword_sym[col]);

	}

	error = 0;
	for(i = 0;i < (U);i ++){
#pragma HLS pipeline II=1
		if(encoded_sym[i + U] != codeword_sym[i + U])
			error++;
	//printf("updated error %d",error);
	}


	//if(error == 0) {

	//return 0;
	//}

	 */

	minmax_label2:while(iter < MAX_ITERATION){
		minmax_label3:for(row = 0; row <U; row++){
			// check node processing: update check node message BETA
#pragma HLS pipeline II=50
			int rowweight = 6;
			minmax_label4:for(index_c = 0, col = col_row1[row][0]; index_c < rowweight; index_c++, col = col_row1[row][index_c]){

				if(index_c == 0){
						minmax_label5:for(a = 0; a < Q; a++){
						F[a][0] = ALPHAmn_1[row][col][mul_gf(inv_gf(H1[row][col]),a)];
						last_col = col_row1[row][rowweight - 1];
						B[a][rowweight - 1] = ALPHAmn_1[row][last_col][mul_gf(inv_gf(H1[row][last_col]),a)];
					}
				}
				else{

					index_p = index_c - 1;
					col_p = col_row1[row][index_p];
					minmax_label323:for(c = 0; c < Q; c++){
						min_F = max(F[c][index_c - 1],ALPHAmn_1[row][col][0]);
						minmax_label7:for(b = 0; b < Q; b++){
							a = sub_gf(c,mul_gf(H1[row][col],b));
							max_F = max(F[a][index_c - 1], ALPHAmn_1[row][col][b]);
							min_F = min(min_F, max_F);
				}
						F[c][index_c] = min_F;
					}
					index_B = rowweight - index_c - 1;
					col_B = col_row1[row][index_B];
					index_p = index_B + 1;
					col_p = col_row1[row][index_p];

					minmax_label8:for(c = 0; c < Q; c++){

						min_B = max(ALPHAmn_1[row][col_B][0],B[c][index_B + 1]);
						minmax_label9:for(b = 0; b < Q; b++){
							a = sub_gf(c,mul_gf(H1[row][col_B],b));
							max_B = max(B[a][index_B + 1], ALPHAmn_1[row][col_B][b]);
							min_B = min(min_B, max_B);
						}
						B[c][index_B] = min_B;

					}
				}
			}
			minmax_label10:for(index_c = 0, col = col_row1[row][0]; index_c < rowweight; index_c++, col = col_row1[row][index_c]){
	//2. update BETA values from B and F values
				if(index_c == 0){
					minmax_label11:for(a = 0; a < Q; a++){
						BETAmn_1[row][col][a] = B[mul_gf(H1[row][col],a)][index_c + 1];
					}
				}
				else if(index_c == (rowweight2[row] - 1)){
					minmax_label12:for(a = 0; a < Q; a++){
						BETAmn_1[row][col][a] = F[mul_gf(H1[row][col],a)][index_c - 1];

					}
				}
				else{

					minmax_label13:for(c = 0; c < Q; c++){
//#pragma HLS pipeline II=1
						min_value = max(F[mul_gf(H1[row][col],c)][index_c - 1], B[0][index_c + 1]);
						minmax_label14:for(b = 0; b < Q; b++){
//#pragma HLS pipeline II=1
//#pragma HLS pipeline

							a = add_gf(b,mul_gf(H1[row][col],c));
							max_value = max(F[a][index_c - 1],B[b][index_c + 1]);
							min_value = min(max_value, min_value);
						}
						BETAmn_1[row][col][c] = min_value;
						//printf("BETAMAN[%d][%d][%d]=%d\n",row,col,c,BETAmn_1[row][col][c]);

					}
				}
			}
		}
	/*	//printf("check node processing finished\n");
int i,j,k;
		for(int i=0; i<U;i++)
		{
			for(j=0;j<V;j++)
			{
				for (k=0;k<Q;k++)
				{
					printf("BETAMAN[%d][%d][%d]=%d\n",i,j,k,BETAmn_1[i][j][k]);

				}
			}
		}
*/


		minmax_label15:for(col_v = 0; col_v < V; col_v ++){

	// 	 processing: update variable node message ALPHA
#pragma HLS pipeline II=70
			index_v = 0; 
			row_v = row_col1[0][col_v];
			minmax_label132:while(index_v < (3)){
				row_v = row_col1[index_v][col_v];
		minmax_label17:for(a = 0; a < Q; a++){
			temp = 0;
					index_ = 0;
					row_ = row_col1[0][col_v];
						minmax_label18:while(index_<3){
						row_ = row_col1[index_][col_v];
						if(index_ != index_v){
							temp += BETAmn_1[row_][col_v][a];
						}
						index_++;
					}
					ALPHA_t[a] = temp + GAMMAn_[a][col_v];
				}
				min_index = 0;
				minmax_label19:for(a = 1; a < Q; a++){

					if(ALPHA_t[a] < ALPHA_t[min_index]){
						min_index = a;
					}
				}
				minmax_label20:for(a = 0; a < Q; a++){
#pragma HLS unroll
					ALPHAmn_1[row_v][col_v][a] = ALPHA_t[a] - ALPHA_t[min_index];
	printf("ALPHAMAN[%d][%d][%d]=%d\n",row_v,col_v,a,ALPHAmn_1[row_v][col_v][a]);
				}
				index_v++;
			}
		}


/*
		printf("variable node processing finished\n");
		//FILE *fp66 = fopen("C:/Users/mtech/Desktop/Q16/data/Gamman_post", "w+");

		minmax_label21:for(col = 0; col < V; col ++){ // post processing
			minmax_label22:for( a = 0; a < Q; a++){
				temp = 0;
				index_ = 0;
				row_ = row_col1[0][col];
				minmax_label723:while(index_ <1 ){
					row_ = row_col1[index_][col];
					temp += BETAmn_1[row_][col][a];
					index_++;
				}
				GAMMAn_post[a][col] = temp + GAMMAn[a][col];
			}
			//fprintf(fp66, "%d ",GAMMAn_post[a][col]);
		}
		//fclose(fp66);
		printf("post processing finished\n");
		minmax_label24:for(col = 0; col < V; col ++){// tentative decoding
			a = 0;
#pragma HLS unroll
			minmax_label25:for(i = 0; i < Q; i++){
				if(GAMMAn_post[i][col] < GAMMAn_post[a][col]){
#pragma HLS unroll
					a = i;
				}
			}
			codeword_sym[col] = a;
			printf("codeword_sym %hhu",codeword_sym[col]);

		}
		error = 0;
		for(i = 0;i < (U);i ++){
			if(encoded_sym[i + U] != codeword_sym[i + U])

#pragma HLS unroll
				error++;
		}
		printf("error =%d ",error);

		if(error == 0){
			return 0;
		}
*/
		iter ++;
	}

	int k;
			for(int i=0; i<U;i++)
			{
#pragma HLS unroll
				for(j=0;j<V;j++)
				{
#pragma HLS unroll
					for (k=0;k<Q;k++)
					{
#pragma HLS unroll
						return BETAmn_1[i][j][k];

					}
				}
			}
	//return 0;
}
